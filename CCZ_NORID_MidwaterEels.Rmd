```{r}
#Linear models to examine differences in vertical distribution between day/night and C5b/C5e
head(dat)
names(dat)
...
# Fit linear models
lm_nemi <- lm(depth ~ day_night, data = dat_nemi)
lm_serri <- lm(depth ~ day_night, data = dat_serri)
...
# Prepare data
dat<-read.csv("/cloud/project/Midwater Eels/Eel_distribution_data.csv")
...
pdf("gamm4_diagnostic_plots_DHARMa_nbehavior.pdf")
plot(simres)
dev.off()
...
dat <- read.csv("/cloud/project/Midwater Eels/Prey_transect_data.csv", header = TRUE, sep = ",", row.names=1)
head(dat)
...
dat <-read.csv("/cloud/project/Midwater Eels/Prey_diversity_data.csv", header = TRUE, sep = ",")
head(dat)
...
pdf("/cloud/project/Midwater Eels/Shannon_by_four_depth_and_daytime_bins.pdf")
...
dat <- read.csv("/cloud/project/Midwater Eels/Prey_transect_data_indsp_dn.csv", header = TRUE, sep = ",", row.names = 1)
indval <- multipatt(dat, dat$group, control = how(nperm=999))
summary(indval)
str(indval)

#do the same with depth groups, with shallow = 1 and deep = 2
dat <- read.csv("/cloud/project/Midwater Eels/Prey_transect_data_indsp_sd.csv", header = TRUE, sep = ",", row.names = 1)
indval <- multipatt(dat, dat$group, control = how(nperm=999))
summary(indval)
```
